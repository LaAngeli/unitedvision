...
<?= $form->field($addForm, 'url')->textInput(['maxlength' => true]) ?>

<?= Html::activeLabel($addForm, 'image_desktop', ['class' => 'img-upload-label'])  ?>
<div class="img-upload-wrapper d-flex flex-column align-items-center " data-prev='upload-image-inp-1'>
    ...
    <?= $form->field($addForm, 'image_desktop')->fileInput(['id' => 'upload-image-inp-1', 'class' => 'inp-hidden'])->label(false) ?>
</div>

<?= Html::activeLabel($addForm, 'image_mobile', ['class' => 'img-upload-label'])  ?>
<div class="img-upload-wrapper d-flex flex-column align-items-center " data-prev='upload-image-inp-2'>
    <div class="prev-area d-flex flex-row flex-wrap align-items-center justify-content-center ">
        <div class="preview">
            <img id="img-upload-preview" class="img-upload-preview" src="<?= Image::getFile([
                                                                                'filePath' => $addForm->filePath,
                                                                                'file' => $addForm->image_mobile
                                                                            ]) ?>">
            <p class="file-name-preview"></p>
        </div>
    </div>
    ...
    <?= $form->field($addForm, 'image_mobile')->fileInput(['id' => 'upload-image-inp-2', 'class' => 'inp-hidden'])->label(false) ?>
</div>
